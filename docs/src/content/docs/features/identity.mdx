...
```
All journals in vault
  │
  ├── Global journals      → always visible
  ├── Person journals      → visible only if ownerPath matches registered user
  └── Group journals       → visible only if registered user is a group member

Identity disabled?
  → only global journals shown
```
