...
```
                ...
                ┌─────────────┐
                │ Bob's PC    │           ┌──────────────────┐
                │ Device ID:  │──────────→│ Person Note:     │
                │  ghi-789    │           │ Bob Jones.md     │
                └─────────────┘           └──────────────────┘
```
...
- Future: shared journal entries attributed to group members

## Resolution chain

When determining the effective settings for a periodic note, the plugin walks a 4-layer chain. Each layer can partially override the layer below — only non-undefined fields take effect.
...
Priority
  ...
  │   │    Per-device overrides      │  Highest priority
  │   ├─────────────────────────────┤
  │   │ 2. Person note frontmatter  │  periodic: block in person note
  │   │    Per-person overrides      │
  │   ├─────────────────────────────┤
  │   │ 3. Settings-based overrides  │  personPeriodicOverrides array
  │   │    Configured in settings UI │  in plugin settings
  │   ├─────────────────────────────┤
  │   │ 4. Vault-wide config         │  settings.periodic[periodicity]
  │   │    Default for everyone      │  Lowest priority
  │   └─────────────────────────────┘
  │
```
